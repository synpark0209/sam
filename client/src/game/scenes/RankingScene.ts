...
export class RankingScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(GAME_W / 2, 20, '랭킹', {
      fontSize: '24px', color: '#ffd700', fontStyle: 'bold',
    }).setOrigin(0.5);

    // 탭 버튼
    const tabStyle = (active: boolean) => ({
      fontSize: '16px', color: active ? '#ffffff' : '#888888',
      backgroundColor: active ? '#3366aa' : '#1a1a3a',
      padding: { x: 20, y: 8 },
    });

    const campaignTab = this.add.text(GAME_W / 2 - 80, 55, '캠페인', tabStyle(true))
      .setOrigin(0.5).setInteractive({ useHandCursor: true });
    const pvpTab = this.add.text(GAME_W / 2 + 80, 55, 'PvP', tabStyle(false))
      .setOrigin(0.5).setInteractive({ useHandCursor: true });
  ...
  private renderCampaignRanking(data: RankingEntry[]): void {
    const header = this.add.text(40, 90, '순위    이름              레벨    진행도', {
    ...
    data.forEach((entry, i) => {
      ...
      const txt = this.add.text(40, y,
        `${rank}      ${name}     Lv.${entry.maxLevel}      ${entry.currentChapterId} - ${entry.currentStageIdx + 1}`,
        { fontSize: '14px', color: i < 3 ? '#ffd700' : '#cccccc' },
      );
      this.contentGroup.add(txt);
    });

    if (data.length === 0) {
      const empty = this.add.text(GAME_W / 2, 150, '데이터가 없습니다', {
        fontSize: '14px', color: '#666666',
      }).setOrigin(0.5);
  ...
  private renderPvpRanking(data: PvpRankingEntry[]): void {
    const header = this.add.text(40, 90, '순위    이름              ELO     전적', {
      fontSize: '13px', color: '#888888',
    });
    this.contentGroup.add(header);
    ...
    if (data.length === 0) {
      const empty = this.add.text(GAME_W / 2, 150, 'PvP 전적이 없습니다', {
        fontSize: '14px', color: '#666666',
      }).setOrigin(0.5);
      this.contentGroup.add(empty);
    }
